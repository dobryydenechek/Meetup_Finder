        ...
        for event in longpoll.listen():
            ...
            if event.type == VkEventType.MESSAGE_NEW:
                ...
                if event.to_me:
                    ...
                    print(request)
                    print(request.lower() == "старт" or request.lower() == "start" or request == "Начать")
                    ...
                    elif request.lower() == 'что означают тэги?':
                        write_msg(event.user_id, 'Фронтенд — все, что браузер может читать, выводить на экран и / или запускать. То есть это HTML JS CSS\n\nbackend - основной функционал сайта, который не виден обычным пользователям, но он есть в основном коде.\n\nСистемный администратор представляет собой специалиста, который обеспечивает нормальную работу компьютерной техники, локальных сетей, ПО.\n\nProject product - Product — это нечто осязаемое, он не ограничен во времени, его развитие продолжается бесконечно (когда оно останавливается, продукт начинает умирать).\n\nВ то время как Project — это процесс, имеющий четкие рамки (временные, бюджетные).\n\nData Science - это наука о работе с большими данными.\n\nгеймдев - разработка компьютерных игр.\n\nmobile - разработка программ для мобильных устройств.\n\nТестирование QA (тестирование программного обеспечения) - это проверка программного продукта на ожидаемый результат и на реальный.\n\nкарьера - мероприятия, где вам подробно расскажут о карьерной лестнице и как правильно по ней идти.\n\nклиенты - мероприятия, для улучшения навыков работы с клиентами.\n\nРобототехника — отдел прикладной науки, который занимается проектированием, производством и применением автоматизированных технических систем — роботов.', a)
